...
from .cityscapes_foggy_eval import foggy_cityscapes_eval
...
class foggycityscapes(imdb):
    def __init__(self, image_set, root_path=None):
        imdb.__init__(self, 'foggycityscapes_' + image_set)
        ...
        self._image_path = os.path.join(self._data_path, 'leftImg8bit_foggy', image_set)
        ...
        assert os.path.exists(self._image_path), "foggy cityscapes image path not exists"
        assert os.path.exists(self._label_path), "foggy cityscapes label path not exists"



    def _get_default_path(self):
        return os.path.join(cfg.DATA_DIR, 'foggycityscapes')
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_foggycityscapes_annotation(index)
    ...
    def _load_foggycityscapes_annotation(self, index):
    ...
    def _get_foggycityscapes_results_file_template(self):
        filename = self._get_comp_id() + '_det_' + self._image_set + '_{:s}.txt'
        filedir = os.path.join(self._data_path, 'results', 'foggycityscapes', 'Main')
        if not os.path.exists(filedir):
            os.makedirs(filedir)
        path = os.path.join(filedir, filename)
        return path
    
    def _write_foggycityscapes_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue

            print("Writing {} cityscapes result file".format(cls))
            filename = self._get_foggycityscapes_results_file_template().format(cls)
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        for i, cls in enumerate(self._classes):
            if cls == '__background__':
                continue
            filename = self._get_foggycityscapes_results_file_template().format(cls)
            rec, prec, ap = foggy_cityscapes_eval(
                filename, annopath, os.path.join(self._data_path, 'leftImg8bit_foggy', self._image_set), cls, cachedir, ovthresh=0.5
            )
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        self._write_foggycityscapes_results_file(all_boxes)
