...
def fullycityscapes_eval(detpath, annopath, imagesetdir, classname, cachedir, ovthresh=0.5):
    # import pdb; pdb.set_trace()
    if not os.path.isdir(cachedir):
        os.mkdir(cachedir)
    cachefile = os.path.join(cachedir, "fullycityscapes_annots.pkl")
    ...
    ap = fullycityscapes_ap(rec, prec)
...
def fullycityscapes_ap(rec, prec):
